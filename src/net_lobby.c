...
void LbNetwork_SendFrontendChatMessage(int player_id, const char *message)
{
...
static TbBool frontend_exchange_complete(void *server_buf, size_t frame_size)
{
    if (frontend_menu_state == FeSt_START_MPLEVEL) {
        return true;
    }
    ...
    if (frontend_menu_state == FeSt_NETLAND_VIEW) {
        int selected_level_number = get_selected_level_number_from_packets(screen_packets, netstate.max_players);
        return selected_level_number > SINGLEPLAYER_NOTSTARTED;
    }
    return false;
}
...
{
    return exchange_frame_block(NETMSG_FRONTEND, send_buf, server_buf, frame_size, frontend_exchange_complete, network_yield_draw_frontend);
}
