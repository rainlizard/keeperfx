...
#define TURN_SPEED_UP -1000000
#define TURN_SLOW_DOWN 1000000
...
static TbClockMSec last_speed_adjust_send = 0;

static void send_speed_adjust_if_due(void)
{
    if (netstate.my_id != SERVER_ID) {
        return;
    }
    TbClockMSec current_time = LbTimerClock();
    if (last_speed_adjust_send != 0 && current_time - last_speed_adjust_send < SPEED_ADJUST_INTERVAL) {
        return;
    }

    last_speed_adjust_send = current_time;
    char *write_pos = begin_net_message(NETMSG_GAMEPLAY_SPEED_ADJUST);
    ...
    for (NetUserId peer_id = 0; peer_id < netstate.max_players; peer_id += 1) {
        ...
        netstate.sp->sendmsg_single_unsequenced(peer_id, netstate.msg_buffer, message_size);
    }
}
...
void LbNetwork_SendGameplayChatMessage(int player_id, const char *message)
{
...
TbError process_network_speed_adjust_message(NetUserId source, const char *buffer, size_t buffer_size)
{
    if (buffer_size != sizeof(GameTurn)) {
        WARNLOG("Invalid gameplay speed adjust message from peer %i (%u bytes)", source, (unsigned)buffer_size);
        return Lb_OK;
    }
    if (source != SERVER_ID) {
        return Lb_OK;
    }
    GameTurn predicted_turn;
    memcpy(&predicted_turn, buffer, sizeof(predicted_turn));
    if (turns_per_second > 0) {
        uint32_t ping = GetPing(SERVER_ID);
        predicted_turn += (GameTurn)(((uint64_t)ping * (uint64_t)turns_per_second + 1999) / 2000);
    }
    GameTurnDelta turn_delta = (GameTurnDelta)(predicted_turn - get_gameturn());
    multiplayer_speed_adjustment_ns = 0;
    if (turn_delta > 0) {
        multiplayer_speed_adjustment_ns = TURN_SPEED_UP;
    } else if (turn_delta < 0) {
        multiplayer_speed_adjustment_ns = TURN_SLOW_DOWN;
    }
    return Lb_OK;
}
...
static void send_player_repair_history(PlayerNumber player)
{
    ...
    if (!network_player_active(player)) {
        return;
    }
    ...
    MULTIPLAYER_LOG("Sending unreliable compressed gameplay repair history for player=%d to all clients (%lu -> %lu bytes)",
        (int)player, (unsigned long)packet_history_size, (unsigned long)compressed_size);
    for (NetUserId peer_id = 0; peer_id < netstate.max_players; peer_id += 1) {
        if (!can_send_to_peer(peer_id)) {
            continue;
