...
static TbBool all_human_players_sent_startup_sync(const struct StartupSyncPacket startup_sync_packets[MAX_NET_USERS])
{
    for (int i = 0; i < MAX_NET_USERS; i++) {
        if (net_player_info[i].network_user_active && !startup_sync_packets[i].startup_sync_packet_valid) {
            return false;
        }
    }
    return true;
...
static TbError exchange_startup_sync(void *send_buf, void *server_buf, size_t frame_size)
{
    return exchange_frame_block(NETMSG_STARTUP_SYNC, send_buf, server_buf, frame_size, NULL, network_yield_draw_gameplay);
}

static TbBool net_startup_sync_exchange_and_apply(void)
{
    memset(s_startup_sync_packets, 0, sizeof(s_startup_sync_packets));
    if (exchange_startup_sync(&s_local_startup_sync, s_startup_sync_packets, sizeof(struct StartupSyncPacket)) != Lb_OK) {
        ERRORLOG("Startup sync exchange failed");
        return false;
    }
    if (!all_human_players_sent_startup_sync(s_startup_sync_packets)) {
...
TbBool init_players_network_game(void)
{
    ...
    if (!net_startup_sync_exchange_and_apply()) {
        return false;
    }
    return true;
}
...
void process_disconnected_network_players(void)
{
    ...
    for (int player_index = 0; player_index < MAX_NET_USERS; player_index++) {
        ...
        if (!player_exists(player) || player_index == my_player_number || (!host_disconnected && network_player_active(player_index))) {
            continue;
        }
